...
def compute_scale_zp(rmin, rmax, qType, quantize_range):
    if qType == onnx_proto.TensorProto.INT8:
        ...
        scale = (float(max_range) * 2) / quantize_range if not math.isclose(max_range, 0, abs_tol=1e-8) else 1
        zero_point = 0
    elif qType == onnx_proto.TensorProto.UINT8:
        scale = (float(rmax) - rmin) / quantize_range if not math.isclose(rmin, rmax, abs_tol=1e-8) else 1
