...
class ONNXQuantizer:
    def __init__(self, model, per_channel, reduce_range, mode, static, weight_qType, input_qType, tensors_range,
        ...
        print(self.quantization_params)
    ...
    def check_opset_version(self):
        ...
        if opset_version < 10:
            opset_version = 12
            logging.warning(
                "The original model opset version is {}, which does not support quantization. Please update the model to opset >= {ver}. Updating the model automatically to opset {ver}. Please verify the quantized model."
                .format(opset_version, ver=opset_version))
            self.model.model.opset_import.remove(ai_onnx_domain[0])
            self.model.model.opset_import.extend([onnx.helper.make_opsetid("", opset_version)])
    ...
    def calculate_quantization_params(self):
        ...
        def adjust_node(node):
            # Note that if we have Relu -> MaxPool then we don't need to adjust Relu afterwards
            # But if we have MaxPool -> Relu then we would need to adjust MaxPool
            if node.op_type not in ['Clip', 'Relu', 'MaxPool']:
                return
            if not self.should_quantize(node):
                return
            nodes_with_same_input = self.model.input_name_to_nodes()[node.input[0]]
            if len(nodes_with_same_input) != 1:
                return
            if node.input[0] not in self.tensors_range.keys() or node.output[0] not in self.tensors_range.keys():
                return
            if self.tensors_range[node.input[0]] != self.tensors_range[node.output[0]]:
                self.tensors_range[node.input[0]] = self.tensors_range[node.output[0]]
                prev_node = self.model.output_name_to_node()[node.input[0]]
                nodes_to_readjust.append(prev_node)

        for node in self.model.nodes():
            adjust_node(node)
        while len(nodes_to_readjust) != 0:
            adjust_node(nodes_to_readjust.pop())
